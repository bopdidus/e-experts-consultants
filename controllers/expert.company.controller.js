...
const { ExpertmoraleValidation, loginValidation, ExpertCompanyUpdateValidation } = require('../middlewares/validation');
...
module.exports = {
    register: async function(req, res){
        ...
        let {error} = ExpertmoraleValidation(req.body);

        if(error) return res.status(400).send(error.details[0].message);
    
        //CHECKING IF USER IS ALREADY IN THE DATABASE
         let idCard = await models.Expert.findOne({where:{email: req.body.email}})
        
          if(idCard) return res.status(401).send("Expert existant");
        ...
        */
       // ActivationAccount(user.email,'expert-company/activation-account', user.firstname+" "+user.lastname);
       try {   
           async.waterfall([
               function(done){
     ...
     profil: async function(req, res){
        ...
        let {error} = ExpertCompanyUpdateValidation(req.body);

        if(error) return res.status(400).send(error.details[0].message);
       let expert = await models.Expert.findOne({where: { email: id.email }});

            expert.company_name = req.body.company_name,
            expert.trade_id = req.body.trade_id,
            expert.niu= req.body.niu,
            expert.email = req.body.email,
            expert.phoneNumber = req.body.phoneNumber,
            expert.social_reason = req.body.social_reason,
            expert.websie = req.body.website,
            ...
            if (expert) {
                ...
                res.send({
                    message: `Cannot update Expert with id=${id.email}. Maybe Expert  was not found or req.body is empty!`
                });
            }
